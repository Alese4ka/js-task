...
wrapper.onkeyup = e => {
    if (e.key == 'Enter') {
        ...
        .then(function (data) {
            ...
            document.querySelector('.weather__temp').innerHTML = Math.round(data.main.temp - 273) + ' C&deg;';
...
function changeTemp() {
    if(switchStatus == true){
        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${weatherCity.innerHTML}&appid=ea7f2ec7419b33064accc22aac5169a6`)
        .then(function (resp) {return resp.json()})
        .then(function (data) {
            document.querySelector('.weather__temp').innerHTML = Math.round(data.main.temp) + ' F';
            switchStatus = false;
        })
        .catch(function () {
            //Обрабатываем ошибки
